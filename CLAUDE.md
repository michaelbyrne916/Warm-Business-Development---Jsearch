...
| 2026-06-09 | WF2: Contact Discovery | **Hunter.io location HARD gate.** Converted the 2026-06-05 soft TLD penalty into a hard gate in `Score & Rank Contacts`: any clear non-US signal (email ccTLD, Hunter source-URL ccTLD, or Hunter org-level `country` when present) forces `confidence_score=0` → Confidence Gate routes to `needs_review`, WF3 not triggered. ccTLD penalty retained for *ranking* only (prefer a US contact when both exist). Org-HQ-state vs job-state recorded as a SOFT flag (`org_state_ne_XX`) only — never blocks (Mike's call 2026-06-09; Hunter HQ state ≠ contact state, would false-reject multi-site US employers). Same gate mirrored into the cleanup branch: `Find Unprocessed Contacts` now carries `contact_location_flag`; `Dedup Manual Against Sent History` hard-fails non-US contacts (stored flag OR re-derived email ccTLD) to `Log Skipped Contacts` (`matched_in=location_gate`), closing the cleanup-branch location bypass. 3 Code nodes edited, no topology change (49 nodes). Validation: 0 new errors (4 pre-existing Code-node false positives on untouched nodes). Canonical export `workflows/wf2-contact-discovery.json` refreshed (was stale since 2026-05-26). |
...
- **WF2 location gate — `.com` recall gap** (originated 2026-06-09). The 2026-06-09 hard location gate is signal-based (email ccTLD, Hunter source-URL ccTLD, org-level `country`). Hunter.io returns **no per-contact location**, so the gate **cannot** catch a non-US contact at a `.com` domain with no other signal (e.g., a London exec at `@globalcorp.com` with US-looking sources). Authoritative per-contact geo requires Apollo/PDL enrichment — deferred until direct-hire revenue justifies the spend (consistent with the 2026-06-05 note). Until then, the gate is a strong-but-leaky filter and `needs_review` rows still warrant a human glance for geography.
...
- **Hunter.io domain-search returns NO per-contact location:** Verified against a live response (WF2 exec 13092, 2026-06-09). Each `data.emails[]` object carries `value / confidence / sources[] / position / seniority / department / linkedin` but **no** country/state/city. The only geo is org-level `data.country` / `data.state` / `data.city` — that is company **HQ**, not the contact, and is frequently **absent** entirely (notably when the query falls back to `company=<name>` because `company_domain` is blank, as in the sampled response). Consequence: any "is this contact in the US / in the job's state" filter built on Hunter alone is **signal-based** (email ccTLD, source-URL ccTLD, org country) and cannot catch non-US contacts on `.com`. Gating on org HQ state is wrong (false-rejects distributed US employers) — keep it a soft flag. Authoritative per-contact geography needs Apollo/PDL enrichment.
