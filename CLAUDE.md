...
- **WF3 parsing failure root cause** (originated 2026-05-26). WF3's LLM (GPT-4o-mini) deterministically fails to produce valid JSON output for certain company data. Top offenders: Virta Health (34x), Highmark Health (17x), Cardinal Health (17x), G2i Inc. (12x), Lumen (12x), Pearson (11x), EchoStar (10x), GovCIO (10x), Epic (9x), DriveTime (8x). Hypotheses: long `job_description` payloads exceeding token budget; non-ASCII characters; brittle prompt template; missing JSON schema enforcement.
  - **Cycling stopped 2026-05-26** by WF3 root-cause fix (parse failures route to Errors tab instead of writing placeholder OQ rows).
  - **Still open:** investigate the underlying parse failures themselves — Errors tab now has actionable diagnostic data (`opportunity_id`, `company_name`, `contact_email`, `raw_excerpt`, `llm_provider`) per failure. Suggested first step: review 5 recent Errors-tab entries for top offender companies and compare prompts/responses against a known-working case.
...
- **WF3 Claude branch JSON enforcement** (originated 2026-05-26). The OpenAI branch uses `response_format: {type: "json_object"}`. The Claude branch has no equivalent enforcement — if `LLM_PROVIDER` flips to `anthropic`, parsing failures could spike. Lowest-disruption fix is the Anthropic tool-use API with a JSON schema tool; deferred until/unless we switch providers.
